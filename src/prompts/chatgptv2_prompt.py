...
class ChatGptV2Prompt:
    def get_prompt(
        ...
        for dialog_id in random_dialog_ids:
            ...
            contexts_with_turn_row_type_1.extend(turn_row_type_1_rows['context'].tolist())
            targets_with_turn_row_type_1.extend(turn_row_type_1_rows['target'].tolist())

            formatted_apicall_examples = []

            for context, target in zip(contexts_with_turn_row_type_1, targets_with_turn_row_type_1):
        ...
        prompt_text += "\n\n".join([
            ...
            '\n', 
            'For intents that are not transactional i.e have isTransactional as 0(False), if you have all the required slots filled up; make the api call promptly and do not ask the user again for reconfirmation",\n', 
            '\n', 
            'For intents that have isTransactional as 1(True), follow the following instructions_",\n', 
            '\n', 
